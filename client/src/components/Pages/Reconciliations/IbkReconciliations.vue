<template>
    ...
    <TableLite :is-slot-mode="true" :isLoading="tableConfig.isLoading" :title="'IBK Reconciliations'"
    ...
    <Modal v-model="isModalVisible" :title="'Details'" :size="'lg'">
        <div v-if="currentReconciliation" class="space-y-6">
            ...
            <div class="flex justify-between items-center">
                <h2 class="text-2xl font-bold text-indigo-600">#Operación: {{ currentReconciliation.at.operation_number }}</h2>
                <ProgressCircle :score="currentReconciliation.score" size="sm" />
            </div>

            <div class="grid grid-cols-1 gap-4">
                <!-- IBK Details -->
                <div class="card p-4 bg-green-200 text-blue-700 shadow-md rounded-lg">
                    <div class="space-y-2">
...
<script setup>
import { getIbkReconciliations, getIbkReconciliation } from "../../../api/reconciliationApi";
...
const { tableConfig, fetchTableData } = useTable(getIbkReconciliations, initialColumns);
...
const openModal = async (id) => {
    ...
    const response = await getIbkReconciliation(id);
...
const filterRows = computed(() => {
    return tableConfig.rows.filter(x => x.ibk.beneficiary.toLowerCase().includes(searchTerm.value.toLowerCase()));
});
