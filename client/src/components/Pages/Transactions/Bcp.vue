...
<script setup>
import { getBcps, changeStatus, addObservations } from "../../../api/bcpApi";
...
const openModal = (id = null) => {
  ...
  const bcp = tableConfig.rows.find((row) => row.id === id);
  if (bcp) {
    observations.value = bcp.observations || '';
...
const handleSubmit = async () => {
  try {
    await addObservations(currentBcpId.value, observations.value);
